 ...
 * @date 2020/10/5
 */
public class MP14Acc extends MP implements MetamorphicPattern {
//
//    @Override
...
//                            System.out.println(tempList.get(k-1));
    ...
    public static List<String> getFullFS(List<String> followUpSeq,List<String> fullSS){
        List<String> fullFS = new ArrayList<>();

        for (String sourceInfo:fullSS){

            for (String followInfo:followUpSeq){
                if (sourceInfo.split(BLANK)[0].equals(followInfo.split(BLANK)[0]+":")){
                    fullFS.add(sourceInfo.split(BLANK)[0]+BLANK
                    +sourceInfo.split(BLANK)[1]+BLANK
                    +"1"+followInfo.split(BLANK)[1]);
                    String[] infos = sourceInfo.split(BLANK);
                    infos[2]="1"+String.valueOf(Integer.valueOf(followInfo.split(BLANK)[1])+1);
    ...
    public static void main(String[] args){
        ...
        MetamorphicPattern mp = new MP14Acc();

        List<String> infofs = mp.followUpSeqWithoutSort(MP.getsourceSeq("F:\\gengning\\workplace\\CMT\\SE\\Account.txt"));
