 ...
 * @date 2020/9/17
 */
public class OBTP5 extends OBTP implements ObtainPattern{
    ...
    public String obtainPattern(List<String> sourceSeq) {
        ...
        /*
        特征5：涉及两个共享变量两个线程，包含一个线程对每个变量的一次写操作，另一个线程对每个变量的一次写操作
         */
        boolean isMatchg5 = false;
        if (threadSet.size() > 1 && variableSet.size() == 2 && unitVSet.size()>3) {
            firstoutter: for (String variable1 : unitVSet) {
                for (String variable2 : unitVSet) {
                    if ((!variable2.equals(variable1)) && variable1.split(" ")[0].equals(variable2.split(" ")[0])) {
                        ...
                        for (int i = 0; i < arrayLists.size(); i++) {
                            ...
                            if (sublist.contains("WRITE " + variable1.split(" ")[1] + " "+variable1.split(" ")[2])
                                    && sublist.contains("WRITE " + variable2.split(" ")[1]+" "+variable2.split(" ")[2])) {
                                        ...
                                        for (String variable3 : unitVSet) {
                                            for (String variable4 : unitVSet) {
                                                if ((!variable3.equals(variable1))
                                                        ...
                                                        && arrayLists.get(j).contains("WRITE " + variable3.split(" ")[1]+" "+variable3.split(" ")[2])
                                                        && arrayLists.get(j).contains("WRITE " + variable4.split(" ")[1]+" "+variable4.split(" ")[2])) {
                                                    isMatchg5 = true;
                                                    break firstoutter;
                                                }
                                            }
    ...
    public static void main(String[] args){
        ...
        ObtainPattern obtp = new OBTP5();
        List<String> sourceSeq = obtp.getTrace("F:\\gengning\\workplace\\CMT\\SE\\AirlineTickets\\AirlineTickets.txt");
