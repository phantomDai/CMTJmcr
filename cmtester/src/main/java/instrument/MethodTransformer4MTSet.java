...
public class MethodTransformer4MTSet extends AdviceAdapter implements Opcodes {
    ...
    protected MethodTransformer4MTSet(MethodVisitor mv, int access, String name, String desc) {
    ...
    private void instrumentField(boolean isBefore, boolean isRead, String fieldOwner, String fieldName, String fieldDesc){
        ...
        if (isBefore) {
            mv.visitMethodInsn(Opcodes.INVOKESTATIC,
                    Instrumentor.EVENT_RECIEVER5, //只需要根据Instrumentor中的常量修改
                    "beforeFieldRead",
                    BOOL_3STRINGS_INT_VOID);
        } else {
            mv.visitMethodInsn(Opcodes.INVOKESTATIC, Instrumentor.EVENT_RECIEVER5, "beforeFieldWrite", BOOL_3STRINGS_INT_VOID,false);
        }
    }
}
